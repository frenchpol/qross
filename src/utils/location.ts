...
export const validatePosition = (position: GeolocationPosition): boolean => {
  ...
  if (isNaN(latitude) || isNaN(longitude) || isNaN(accuracy)) {
    console.warn('Invalid coordinate values:', { latitude, longitude, accuracy });
    return false;
  }
  ...
  if (!position.timestamp || isNaN(position.timestamp)) {
    console.warn('Invalid timestamp:', position.timestamp);
    return false;
  }
